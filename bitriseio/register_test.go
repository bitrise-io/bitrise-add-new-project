...
func TestAppsService_Register(t *testing.T) {
	...
	// happy path
	{
		...
		params := RegisterParams{
			GitOwner:    "github_user",
			GitRepoSlug: "my_repo",
			IsPublic:    true,
			Provider:    "github",
			RepoURL:     "https://github.com/github_user/my_repo.git",
			Type:        "git",
		}
		if slug, err := client.Apps.Register(params); err != nil {
			t.Errorf("want: nil, got: %s", err)
			return
		} else if slug == "" {
			t.Errorf("want: slug, got: %s", slug)
			return
		}
	}
